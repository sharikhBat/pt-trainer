...
export async function POST(request: NextRequest) {
  try {
    const { clientId, datetime } = await request.json();

    if (!clientId || !datetime) {
      return NextResponse.json({ error: 'Client ID and datetime are required' }, { status: 400 });
    }

    const bookingDate = new Date(datetime);

    // Check if the slot is already booked by anyone
    const slotTaken = await isSlotBooked(bookingDate);
    if (slotTaken) {
      return NextResponse.json({ error: 'This slot is no longer available' }, { status: 409 });
    }

    // Check if client already has a booking on this day
    const hasBookingToday = await hasClientBookingOnDate(parseInt(clientId), bookingDate);
    if (hasBookingToday) {
      return NextResponse.json({ error: 'You already have a session booked for this day' }, { status: 409 });
    }

    const booking = await createBooking(parseInt(clientId), bookingDate);
